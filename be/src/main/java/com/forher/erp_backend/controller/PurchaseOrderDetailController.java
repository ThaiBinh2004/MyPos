...
import com.forher.erp_backend.dto.PurchaseOrderDetailResponse;
...
public class PurchaseOrderDetailController {
    ...
    public ResponseEntity<?> getDetailsByPoId(@PathVariable String poId) {
        return ResponseEntity.ok(poDetailService.getDetailsByPoId(poId).stream()
                .map(PurchaseOrderDetailResponse::from).toList());
    }

    @PostMapping
    public ResponseEntity<?> create(@RequestBody PurchaseOrderDetail detail) {
        return ResponseEntity.status(HttpStatus.CREATED).body(
                PurchaseOrderDetailResponse.from(poDetailService.createPurchaseOrderDetail(detail))
        );
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> updateQuantityReceived(@PathVariable String id, @RequestBody PurchaseOrderDetail detail) {
        return ResponseEntity.ok(PurchaseOrderDetailResponse.from(poDetailService.updatePurchaseOrderDetail(id, detail)));
    }
}
